 ...
 * a Garage / S3 image the user might want to inspect (bug-report screenshots,
 * announcement attachments, vehicle photos, etc.) instead of the old
 * `<a target="_blank">` pattern — keeps the user inside the app.
 *
...
export function LightboxImage({
  ...
  src: string;
  alt?: string;
  /** Class applied to the trigger thumbnail. */
  className?: string;
  /** Pass-through for any other img attributes (loading, decoding, etc). */
  ...
  // Lock body scroll + listen for ESC while the lightbox is open.
  useEffect(() => {
    if (!open) return;
    ...
    <>
      ...
      {mounted && open &&
        createPortal(
          ...
          >
            {/* Close button — top-right of the viewport so it's always reachable
                even on a portrait phone where the image is taller than wide. */}
            <button
              type="button"
